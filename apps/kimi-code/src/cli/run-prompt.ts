      ...
      switch (event.type) {
        ...
        case 'turn.ended':
          if (event.reason === 'completed') {
            void (async () => {
              // Flush the buffered assistant message before the end-of-turn
              // policy runs: in stream-json mode the final message is only
              // emitted by finish(), so a long drain/steer wait would otherwise
              // withhold the main turn's result until the run exits.
              outputWriter.flushAssistant();
              try {
                const action = await session.handlePrintMainTurnCompleted();
                if (action === 'continue') {
                  // Stay alive: a still-pending background task will, on
                  // completion, steer the main agent into a new turn whose
                  // events we keep mapping. Do not finish yet.
                  return;
                }
              } catch (error) {
